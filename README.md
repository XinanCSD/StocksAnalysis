...
- 顶部工具栏可选择或搜索股票、输入多个股票代码、切换 5m / 30m / 1h / 2h / 4h / 1D / 1W / 1M / 1Y、切换 K 线 / OHLC / 线型 / 面积图、适应全部数据、返回最新数据及深浅主题。
- 侧边栏显示股票、下载状态、行数与失败信息；失败项可直接点击“重试”。添加时支持逗号、空格或换行分隔；例如 `AAPL, BRK.B`。`BRK.B` 会按 Yahoo 的 `BRK-B` 保存和下载，而 `7203.T` 保持不变。
